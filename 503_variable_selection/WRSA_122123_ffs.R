...
# predictor sets
  ...
  pred_all <- species_data$WRSA %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$WRSA %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$WRSA$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_WRSA.RDS")
