...
# predictor sets
  ...
  pred_all <- species_data$BASA %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$BASA %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$BASA$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_BASA.RDS")
