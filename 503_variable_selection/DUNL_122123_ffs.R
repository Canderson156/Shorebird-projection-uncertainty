...
# predictor sets
  ...
  pred_all <- species_data$DUNL %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$DUNL %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$DUNL$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_DUNL.RDS")
