...
# predictor sets
  ...
  pred_all <- species_data$PESA %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$PESA %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$PESA$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_PESA.RDS")
