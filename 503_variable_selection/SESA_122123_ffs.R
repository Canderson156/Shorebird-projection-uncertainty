...
# predictor sets
  ...
  pred_all <- species_data$SESA %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$SESA %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$SESA$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_SESA.RDS")
