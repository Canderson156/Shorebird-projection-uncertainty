...
# predictor sets
  ...
  pred_all <- species_data$REPH %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$REPH %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$REPH$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_REPH.RDS")
