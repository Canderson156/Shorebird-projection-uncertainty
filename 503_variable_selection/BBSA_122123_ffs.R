...
# predictor sets
  ...
  pred_all <- species_data$BBSA %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$BBSA %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$BBSA$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_BBSA.RDS")
