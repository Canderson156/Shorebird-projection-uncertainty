...
# predictor sets
  ...
  pred_all <- species_data$BBPL %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$BBPL %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$BBPL$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_BBPL.RDS")
