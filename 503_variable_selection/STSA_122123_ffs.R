...
# predictor sets
  ...
  pred_all <- species_data$STSA %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$STSA %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$STSA$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_STSA.RDS")
