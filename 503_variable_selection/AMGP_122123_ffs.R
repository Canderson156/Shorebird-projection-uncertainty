...
# predictor sets
  ...
  pred_all <- species_data$AMGP %>%
    select(all_of(c(all_predictors2)))
  
  pred_clim <- species_data$AMGP %>%
...
vs6 <- ffs(predictors = pred_all, 
           response = species_data$AMGP$presence,
...
saveRDS(selected_vars, "R_objects/selected_vars_AMGP.RDS")
