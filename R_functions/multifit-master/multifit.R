multifit <- function(mod, multief, data, formula = NULL, args = NULL, criterion = "AIC", site_id = NULL, signif = TRUE, alpha = 0.05, plot_est = FALSE,
    print_sum = FALSE, xlab = "Radius [km]", ylab = NULL, labels = NULL, type = "b", pch = c(1, 16)) {

    # Arguments checking
    if (!is.character(mod) || length(mod) != 1)
        stop("Argument mod must be a character of length 1")
    if (!is.character(multief))
        stop("Argument multief must be a character vector")
    if (!is.null(args)) {
        if (!is.character(args))
            stop("Argument args must be NULL or a character vector")
    }
    ...
    if (!is.logical(signif) || length(signif) != 1)
        stop("Argument signif must be logical")
    if (!is.numeric(alpha) || !(alpha > 0 && alpha <= 1) || length(alpha) != 1)
        stop("Argument alpha must be a number between 0 and 1")
    if (!is.logical(print_sum) || length(print_sum) != 1)
        stop("Argument print_sum must be logical")
    if (!is.character(xlab) || length(xlab) != 1)
        stop("Argument xlab must be character of length 1")
    if (!is.null(ylab)) {
        if (!is.character(ylab) || length(ylab) != 1)
            stop("Argument ylab must be NULL or character of length 1")
    }
    if (!is.logical(plot_est) || length(plot_est) != 1)
        stop("Argument plot_est must be logical")
    if (!is.null(site_id)) {
        if (!is.character(site_id) || length(site_id) != 1)
            stop("Argument site_id must be NULL or character of length 1")
    }
    if (!is.null(labels)) {
        if (!is.character(as.character(labels)))
            stop("Argument labels must be NULL or a character/numeric vector")
    }
    if (!is.numeric(pch))
        stop("Argument pch must be numeric")
    ...
    if (!is.function(tryCatch(eval(parse(text = mod)), error = function(e) FALSE))) {
        mods_list <- data.frame(mods = c("lm", "glm", "lmer", "glmer", "glmmadmb", "glmm.admb", "glmmTMB", "lme", "zeroinfl", "hurdle"), pcks = c("stats",
            "stats", "lme4", "lme4", "glmmADMB", "glmmADMB", "glmmTMB", "nlme", "pscl", "pscl"))
        if (mod %in% mods_list$mods) {
            pck <- mods_list$pcks[pmatch(mod, mods_list$mods)]
            if (pck %in% rownames(installed.packages())) {
                load <- readline(prompt = paste("The necessary package ('", pck, "') to run the models with function '", mod, "' is not loaded. Would you like to load it? (Yes = 1; No = 0)\n",
                  sep = ""))
                if (load == "1") {
                ...
                stop(paste("The necessary package ('", pck, "') for function '", mod, "' is not installed. Please install it, load it and reran multifit",
                  sep = ""))
            }
        } else {
    ...
    if (!is.null(args))
        initial.args <- paste(",", paste(args, collapse = ","), sep = "") else initial.args <- NULL
    ...
    lands_table <- function(multief, data) {
        table <- data.frame(spatial_scale = as.character(multief), n = rep(0, length(multief)), min = rep(0, length(multief)), max = rep(0,
            length(multief)), range = rep(0, length(multief)), mean = rep(0, length(multief)), median = rep(0, length(multief)))
    ...
    if (length(mod_errors) != length(multief)) {
        ...
        if (is.numeric(fits.GoF))
            fits.GoF[fits.GoF == Inf | fits.GoF == -Inf] <- NA
        if (any(!is.na(fits.GoF))) {
            ...
            message("The model including the effect '", out, "' was the best model according to the specified criterion (", criterion[1],
                ", ", decision_func, ")")
            ...
            warning(paste("Could not obtain '", criterion[1], "' values, possibly due to fatal errors in all models. 
                    Check them out by typing $Models to the generated object"),
                call. = FALSE)
            plot <- sum <- summ <- NULL
        }
    } else {
