...
selected_vars <- readRDS("R_objects/selected_vars_AMGP.RDS")
...
focal_sp_data <- species_data$AMGP
#selected_vars <- selected_vars$AMGP
...
focal_sp_data2$y_pred = predict(vs2, focal_sp_data2, type="prob")$yes


focal_sp_data2 %>%
  group_by(presence) %>%
  summarize(mean = mean(y_pred))
...
all_predicted_AMGP <- map_dfr(list(
  "m1_predicted",
  "m2_predicted",
  "m3_predicted",
  "m4_predicted",
...
saveRDS(all_predicted_AMGP, "R_objects/predicted_AMGP_0530_20_A.RDS")
